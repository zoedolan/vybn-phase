...
body{font-family:'SF Pro Text',-apple-system,system-ui,'Segoe UI',Roboto,sans-serif;background:#0a0a0f;color:#e0e0e8;display:flex;flex-direction:column;height:100vh;height:100dvh}
...
.transcript{font-size:.8em;color:#6060a0;padding:4px 8px 8px;min-height:0;max-height:80px;overflow-y:auto;transition:all .2s}
...
.btn{width:46px;height:46px;border-radius:50%;border:none;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .15s}
.mic-btn{background:#2a2a4e}
.mic-btn.recording{background:#6a2a2a;animation:pulse 1.5s ease-in-out infinite}
@keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(106,42,42,.4)}50%{box-shadow:0 0 0 12px rgba(106,42,42,0)}}
...
<div class="input-area">
  ...
  <div class="input-row">
    <input type="text" id="ti" placeholder="type or hold mic..." autocomplete="off">
    <button class="btn mic-btn" id="mic" aria-label="Record">
...
let recognition=null,isRec=false,pendingText='';
...
// Speech recognition (Web Speech API — on-device on Android Chrome)
if('webkitSpeechRecognition' in window||'SpeechRecognition' in window){
  ...
  recognition.onresult=function(e){
    ...
    transcript.textContent=pendingText+interim;
    transcript.scrollTop=transcript.scrollHeight;
    sendBtn.style.display=(pendingText.trim()||interim)?'flex':'none';
  };
  recognition.onerror=function(e){stopRec();};
  recognition.onend=function(){if(isRec){try{recognition.start();}catch(e){stopRec();}}};
}
...
function startRec(){
  ...
  try{recognition.start();isRec=true;mic.classList.add('recording');}catch(e){}
}
function stopRec(){
  if(!recognition)return;
  isRec=false;mic.classList.remove('recording');
...
async function send(text){
  if(!text.trim())return;
  transcript.textContent='';pendingText='';sendBtn.style.display='none';ti.value='';
  ti.disabled=true;mic.disabled=true;
  try{
    await fetch(u('/notebook'),{method:'POST',headers:H,body:JSON.stringify({text:text.trim(),author:'Zoe'})});
    await loadEntries();
  }catch(e){}
  ti.disabled=false;mic.disabled=false;ti.focus();
}

sendBtn.addEventListener('click',function(){
  const text=pendingText.trim()||ti.value.trim();
  stopRec();
  send(text);
});
...
function renderEntries(data){
  ...
  let html='';
  for(const day of data.entries){
    html+='<div class="day-label">'+day.date+'</div>';
    const blocks=day.raw.split('\n## ').filter(b=>b.trim());
    for(const block of blocks){
      ...
      const cls=author.toLowerCase()==='zoe'?'zoe':'vybn';
...
async function loadEntries(){
  try{
    const r=await fetch(u('/notebook/recent?days=7'));
